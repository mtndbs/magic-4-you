      ...
      <div className="container mx-auto px-4 md:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <div className="flex items-center gap-2 text-2xl font-script tracking-wider text-gold">
            <img src={logo} alt="Magic 4 You Logo" className="w-10 h-10 object-contain" />
          ...
          <div className="hidden md:flex items-center gap-8">
            <div className="flex gap-6">
              {links.map((link) => (
                <a key={link.name} href={link.href} className="text-sm text-gray-300 hover:text-gold transition-colors">
                  {link.name}
                </a>
              ))}
            </div>
      ...
      {isOpen && (
        <div className="md:hidden bg-dark border-t border-white/5 p-4">
          <div className="flex flex-col gap-4">
            {links.map((link) => (
              <a key={link.name} href={link.href} className="text-gray-300 hover:text-gold transition-colors">
                {link.name}
              </a>
            ))}
            <Button 
